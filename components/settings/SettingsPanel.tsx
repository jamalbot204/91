...
const SettingsPanel: React.FC = memo(() => {
    ...
    const handleInputChange = useCallback((e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => {
        ...
        if (name === "model") {
            ...
            } else if (value !== 'gemini-3-flash-preview') {
                 setLocalSettings(prev => {
                     if (prev.thinkingLevel === 'medium' || prev.thinkingLevel === 'minimal') {
                         return { ...prev, thinkingLevel: 'high' };
                     }
                     return prev;
                 });
            }
    ...
    const handleThinkingLevelChange = useCallback((e: React.ChangeEvent<HTMLSelectElement>) => {
         ...
         setLocalSettings(prev => ({...prev, thinkingLevel: val as 'minimal' | 'low' | 'medium' | 'high'}));
    }, []);
